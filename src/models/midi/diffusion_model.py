...
from src.models.midi.models.transformer_model import GraphTransformer
# from src.models.midi.models.egnn_ablation import GraphTransformer
# print("RUNNING ABLATION")
from src.models.midi.diffusion.noise_model import DiscreteUniformTransition, MarginalUniformTransition
from src.models.midi.diffusion import diffusion_utils
from src.models.midi.diffusion.diffusion_utils import mask_distributions, sum_except_batch
from src.models.midi.metrics.train_metrics import TrainLoss
from src.models.midi.metrics.abstract_metrics import SumExceptBatchMetric, SumExceptBatchKL, NLL
from src.models.midi import utils
import src.models.midi.analysis.visualization as visualizer
import src.models.midi.metrics.abstract_metrics as custom_metrics
from src.models.midi.metrics.molecular_metrics import TrainMolecularMetrics, SamplingMetrics
from src.models.midi.diffusion.extra_features import ExtraFeatures
from src.models.midi.analysis.rdkit_functions import Molecule
from src.models.midi.datasets.adaptive_loader import effective_batch_size

# added imports for compatibility
from src.models.midi.diffusion.distributions import DistributionNodes
...
class CompatibilityDatasetInfos:
    ...
    def __init__(self, dataset_info, include_auxiliary_node_states=False):
        ...
        max_n_nodes = dataset_info['num_nodes_max']
        n_nodes_tens = torch.zeros(max_n_nodes + 1, dtype=torch.long)
        for k, v in n_nodes.items():
            n_nodes_tens[int(k)] = v
        self.nodes_dist = DistributionNodes(n_nodes_tens)

        self.n_nodes = self.nodes_dist.prob
        self.atom_types = torch.tensor(dataset_info['marginals']['x'])
        self.edge_types = torch.tensor(dataset_info['marginals']['edge_attr'])
        # in MiDi, charges are separated atom-wise
        # here they are already aggregated
        self.charges_marginals = torch.tensor(dataset_info['marginals']['node_charges'])
...
class MixedGraphSpatialDenoisingDiffusionModel(GeneratorWithEvaluation):
    ...
    def __init__(
        ...
        #     'edge_adjmat': dataset_info['num_cls_edges'] + 1, # +1 for no-edge
    ...
    def on_evaluation_epoch_end(self, which='valid'):
        ...
        if assignment is not None:
            ...
            if which == 'test' and len(assignment_output) > 2:
        ...
        if which == 'test' and generated_samples is not None:
    ...
    # this is a compatibility method for computing the assigment
    @torch.no_grad()
    def sample(self, num_samples: int, **kwargs):
        return self.sample_n_graphs(
            samples_to_generate = num_samples,
            chains_to_save = 0,
            samples_to_save = 0,
            test = True # set to True to enable all-steps generation
        )
