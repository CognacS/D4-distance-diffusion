...
@clb.reg_other_callbacks.register()
class EMA(Callback):
    """
    ...
    def should_store_parameters(self, step: int) -> bool:
        return step < self.start_step

    def on_train_batch_end(
        self, trainer: "pl.Trainer", pl_module: "pl.LightningModule", outputs: STEP_OUTPUT, batch: Any, batch_idx: int
    ) -> None:
        if self.should_store_parameters(trainer.global_step):
            self._cur_step = trainer.global_step
            self._ema_model_weights = [p.detach().clone() for p in pl_module.state_dict().values()]
        elif self.should_apply_ema(trainer.global_step):
            self._cur_step = trainer.global_step
            self.ema(pl_module)
