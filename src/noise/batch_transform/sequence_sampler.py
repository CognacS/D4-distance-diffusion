...
from src.datatypes.split import merge_batches, merge_edge_batches, merge_subgraphs, get_subgraph
...
def sample_sequences(
    ...
    n0 = batch.num_nodes_per_sample if not hasattr(batch, 'global_n0') else batch.global_n0
    bs = batch.num_graphs

    # determine the maximum length of the sequences
    # which is the max among the max times of the removal process
    max_time = removal_process.get_max_time(n0=n0, data=batch)
    ...
    norm_t = removal_process.normalize_reverse_time(t=curr_batch.global_t, n0=n0, data=curr_batch)
    ...
    should_append = lambda x: (x in selected_indices or (return_first and x == 1)) if num_subsamples >= 0 else True

    mask = torch.ones(bs, dtype=torch.bool, device=batch.x.device)
    ...
    for t in range(1, max_max_time+1):
        ...
        if should_append(t):
            ...
            surv_mask = mask[curr_batch.batch]
            curr_batch = get_subgraph(curr_batch, surv_mask)

        surv_batch: SparseGraph
        remv_batch: SparseGraph
        remv_edges_ab: SparseEdges
        remv_edges_ba: SparseEdges
        surv_batch, remv_batch, remv_edges_ab, remv_edges_ba = removal_process.sample_next(
        ...
        norm_t = removal_process.normalize_reverse_time(t=next_time, n0=n0, data=curr_batch)
        ...
        surv_batch.global_nt = curr_batch.global_nt - remv_batch.global_nt
        surv_batch.global_batch_idx = global_batch_idx

        if should_append(t):
            mask = surv_batch.global_nt > 0
            # alive_masks.append(mask)
            # remove from surv_batch those that are not alive
            # surv_mask = mask[surv_batch.batch]
            # surv_batch = get_subgraph(surv_batch, surv_mask)
            ...
            edges['remv_edges_ba'].append(remv_edges_ba)
            if directed:
                edges['remv_edges_ab'].append(remv_edges_ab)

        curr_batch = surv_batch

    # merge all batches together
    merged_sequences = {
        **{k: merge_batches(v, alive_masks) for k, v in sequences.items()},
        **{k: merge_edge_batches(v, alive_masks) for k, v in edges.items()}
    }

    if also_return_splits:
        sequences['remv_edges_ba'] = edges['remv_edges_ba']
        if directed:
            sequences['remv_edges_ab'] = edges['remv_edges_ab']
...
from src.evaluation.utils.synth import are_isomorphic
